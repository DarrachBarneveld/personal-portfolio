        ...
        >
          <Menu.Items className="no-scrollbar absolute -left-5 top-7 z-50 mt-2 h-60 w-fit  overflow-scroll rounded-md border-2 border-indigo-500 bg-white p-1 px-2 shadow-xl shadow-indigo-950 ring-1 ring-black/5 focus:outline-none">
            <div className="px-1 py-1">
              {DEFAULT_TAGS.map((tag: string, index: number) => (
                <Menu.Item key={tag + index}>
                  {({ active }) => (
                    <button
                      ...
                      className={clsx(
                        `${colorTagVariants[tag][0]} mb-1 w-full rounded-full px-2 py-1 text-left text-[0.7rem] uppercase tracking-wider lg:text-[0.8rem]  
                    ${colorTagVariants[tag][1]} flex-nowrap whitespace-nowrap transition-colors duration-200 hover:bg-gray-800 hover:text-white hover:ring-indigo-500 focus:bg-gray-900 focus:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1 focus:ring-offset-gray-100`,
                      )}
                    >
                      {tag}
                    </button>
